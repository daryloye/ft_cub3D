...
/*   Updated: 2024/11/09 16:29:55 by daong            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	create_fov(t_data *data, float x, float y)
{
	float	start[2];
	float	end[2];
...
int	minimap_player(t_data *data)
{
	...
	x = (data->player->x_pos * wall_length) - (player_length / 2);
	y = (data->player->y_pos * wall_length) - (player_length / 2);
	if (!data->minimap->minimap_player_img)
		data->minimap->minimap_player_img = create_player_image(data);
	mlx_put_image_to_window(data->mlx->mlx_ptr,
		data->mlx->win_ptr, data->minimap->minimap_player_img, x, y);
	create_fov(data, x, y);
	return (0);
}
